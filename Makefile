...
ifndef ROLE
ifndef IGNORE
	ansible-playbook -i $(DIR)/inventory $(DIR)/playbook.yml --limit ${PROFILE} --diff --check --ask-become-pass $(ARG)
else
	ansible-playbook -i $(DIR)/inventory $(DIR)/playbook.yml --limit ${PROFILE} --diff --check --ask-become-pass $(ARG) --skip-tags=$(IGNORE)
endif
else
ifndef IGNORE
	ansible-playbook -i $(DIR)/inventory $(DIR)/playbook.yml --limit ${PROFILE} --diff --check --ask-become-pass $(ARG) -t $(ROLE)
else
	ansible-playbook -i $(DIR)/inventory $(DIR)/playbook.yml --limit ${PROFILE} --diff --check --ask-become-pass $(ARG) -t $(ROLE) --skip-tags=$(IGNORE)
endif
endif
